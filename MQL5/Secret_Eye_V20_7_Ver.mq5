//+------------------------------------------------------------------+
//|           Stochastic Straight Algo Strategy V20.7.mq5           |
...
#property version   "20.70"
#property description "V20.7 - Futures Tick Conversion, Aeron Binary Publisher, JSON Publisher, FOK Orders"
...
int ConvertPointsToFuturesTicks(int points, string futuresSymbol)
{
   ...
   if(futuresSymbol == "6A")           futuresTickSize = 0.00005;   // Australian Dollar: $5.00 per tick
   else if(futuresSymbol == "6B")      futuresTickSize = 0.0001;    // British Pound: $6.25 per tick
   else if(futuresSymbol == "6C")      futuresTickSize = 0.00005;   // Canadian Dollar: $5.00 per tick
   else if(futuresSymbol == "6E")      futuresTickSize = 0.00005;   // Euro FX: $6.25 per tick
   else if(futuresSymbol == "6J")      futuresTickSize = 0.0000005; // Japanese Yen: $6.25 per tick
   else if(futuresSymbol == "6N")      futuresTickSize = 0.00005;   // New Zealand Dollar: $5.00 per tick
   else if(futuresSymbol == "6S")      futuresTickSize = 0.0001;    // Swiss Franc: $12.50 per tick
   
   // CME Equity Index Futures
   else if(futuresSymbol == "ES")      futuresTickSize = 0.25;      // E-mini S&P 500: $12.50 per tick
   else if(futuresSymbol == "NQ")      futuresTickSize = 0.25;      // E-mini Nasdaq: $5.00 per tick
   else if(futuresSymbol == "YM")      futuresTickSize = 1.0;       // E-mini Dow: $5.00 per tick
   else if(futuresSymbol == "RTY")     futuresTickSize = 0.10;      // E-mini Russell 2000: $5.00 per tick
   
   // NYMEX Energy Futures
   else if(futuresSymbol == "CL")      futuresTickSize = 0.01;      // Crude Oil: $10.00 per tick
   else if(futuresSymbol == "NG")      futuresTickSize = 0.001;     // Natural Gas: $10.00 per tick
   
   // COMEX Metal Futures  
   else if(futuresSymbol == "GC")      futuresTickSize = 0.10;      // Gold: $10.00 per tick
   else if(futuresSymbol == "SI")      futuresTickSize = 0.005;     // Silver: $25.00 per tick
   
   else
   {
      // Unknown symbol - use 1:1 conversion as fallback
      PrintFormat("[TICK_CONVERSION] ⚠️ Warning: Unknown futures symbol '%s', using 1:1 point-to-tick conversion", 
                  futuresSymbol);
      return points;
   }
        ...
        if(isExit)
        {
            ...
            if(dealReason == DEAL_REASON_SL || dealReason == DEAL_REASON_TP)
            {
                ...
                if(EnableAeronPublishing)
                {
                    ...
                    {
                        AeronPublishSignal(g_AeronSymbol, g_AeronInstrument, AERON_PROFIT_TARGET,
                                          0, 0, 0, 1, 50.0, AeronSourceTag);
                        Print("[AERON_PUB] ✅ ProfitTarget (short): ", g_AeronSymbol);
                    }
                    else
                    {
                        AeronPublishSignal(g_AeronSymbol, g_AeronInstrument, AERON_SHORT_STOPLOSS,
                                          0, 0, 0, 1, 50.0, AeronSourceTag);
        ...
        if(isExit)
        {
            ...
            if(dealReason == DEAL_REASON_SL || dealReason == DEAL_REASON_TP)
            {
                ...
                if(EnableAeronPublishing)
                {
                    ...
                    {
                        AeronPublishSignal(g_AeronSymbol, g_AeronInstrument, AERON_LONG_STOPLOSS,
                                          0, 0, 0, 1, 50.0, AeronSourceTag);
