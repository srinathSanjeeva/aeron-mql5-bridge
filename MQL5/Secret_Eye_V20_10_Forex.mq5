...
input ENUM_TIMEFRAMES   TimeframeX1 = PERIOD_D1;        // Higher timeframe bias (X1)
input int               K_Period_X1 = 10;               // X1 K Period
input int               D_Period_X1 = 10;               // X1 D Period
input int               Slowing_X1 = 3;                 // X1 Slowing
input ENUM_TIMEFRAMES   TimeframeX2 = PERIOD_M15;       // Lower timeframe trigger (X2)
input int               K_Period_X2 = 10;               // X2 K Period
input int               D_Period_X2 = 10;               // X2 D Period
input int               Slowing_X2 = 3;                 // X2 Slowing
...
static ENUM_X1_BIAS g_x1Bias = X1_BIAS_NONE;
static bool         g_x1BiasReady = false;
static bool         g_waitingForX2Trigger = false;
static datetime     g_lastX1SignalBarTime = 0;
...
bool GetStochasticCrossSignals(int indicatorHandle, bool &bullishCross, bool &bearishCross);
bool GetStochasticDirection(int indicatorHandle, ENUM_X1_BIAS &direction);
void ApplyX1Bias(ENUM_X1_BIAS newBias, bool fromStartup);
...
bool GetStochasticCrossSignals(int indicatorHandle, bool &bullishCross, bool &bearishCross)
{
    ...
    if(!SafeCopyBuffer(indicatorHandle, 0, 1, 3, mainLine, OP_INDICATOR) ||
       !SafeCopyBuffer(indicatorHandle, 1, 1, 3, signalLine, OP_INDICATOR))
    {
        return false;
    }
    ...
    bullishCross = (k0 < d0) && (k1 > d1);
    bearishCross = (k0 > d0) && (k1 < d1);
    return true;
}

bool GetStochasticDirection(int indicatorHandle, ENUM_X1_BIAS &direction)
{
    direction = X1_BIAS_NONE;

    double mainLine[2], signalLine[2];
    if(!SafeCopyBuffer(indicatorHandle, 0, 1, 2, mainLine, OP_INDICATOR) ||
       !SafeCopyBuffer(indicatorHandle, 1, 1, 2, signalLine, OP_INDICATOR))
    {
        return false;
    }

    if(mainLine[0] > signalLine[0])
        direction = X1_BIAS_LONG;
    else if(mainLine[0] < signalLine[0])
        direction = X1_BIAS_SHORT;
...
void ApplyX1Bias(ENUM_X1_BIAS newBias, bool fromStartup)
{
    ...
    if(!g_x1BiasReady)
    {
        g_x1Bias = newBias;
        g_x1BiasReady = true;
        g_waitingForX2Trigger = true;
        PrintFormat("X1 bias initialized: %s%s", newBias == X1_BIAS_LONG ? "LONG" : "SHORT", fromStartup ? " (startup)" : "");
        return;
    }
    ...
    PrintFormat("X1 bias flip detected: %s -> %s", g_x1Bias == X1_BIAS_LONG ? "LONG" : "SHORT", newBias == X1_BIAS_LONG ? "LONG" : "SHORT");
    g_x1Bias = newBias;
    g_waitingForX2Trigger = true;

    UpdateAllPositionStatus();
    if(newBias == X1_BIAS_LONG)
    {
        if(scalpSellOpened || trendSellOpened)
        {
            Print("Closing SELL positions on X1 flip to LONG. Waiting for X2 LONG trigger.");
            CloseAllSellPositions();
            UpdateAllPositionStatus();
        }
    }
    else if(newBias == X1_BIAS_SHORT)
    {
        if(scalpBuyOpened || trendBuyOpened)
        {
            Print("Closing BUY positions on X1 flip to SHORT. Waiting for X2 SHORT trigger.");
...
int OnInit()
{
    ...
    // Initialize X1 and X2 stochastic indicators
    stochHandleX1 = iStochastic(_Symbol, TimeframeX1, K_Period_X1, D_Period_X1, Slowing_X1, MODE_SMA, STO_LOWHIGH);
    if(stochHandleX1 == INVALID_HANDLE)
    {
        HandleError(OP_INIT, GetLastError(), "Failed to initialize X1 stochastic indicator", true);
        return(INIT_FAILED);
    }

    stochHandleX2 = iStochastic(_Symbol, TimeframeX2, K_Period_X2, D_Period_X2, Slowing_X2, MODE_SMA, STO_LOWHIGH);
    if(stochHandleX2 == INVALID_HANDLE)
    {
        HandleError(OP_INIT, GetLastError(), "Failed to initialize X2 stochastic indicator", true);
        return(INIT_FAILED);
    }
    ...
    // Dual-timeframe startup behavior for X1 bias
    g_x1Bias = X1_BIAS_NONE;
    g_x1BiasReady = false;
    g_waitingForX2Trigger = false;
    g_lastX1SignalBarTime = 0;

    if(ImmediateEntryOnLoad)
    {
        ENUM_X1_BIAS startupBias = X1_BIAS_NONE;
        if(GetStochasticDirection(stochHandleX1, startupBias) && startupBias != X1_BIAS_NONE)
        {
            ApplyX1Bias(startupBias, true);
            Print("ImmediateEntryOnLoad=true: X1 bias loaded immediately; waiting for X2 confirmation trigger.");
        }
        else
        {
            Print("ImmediateEntryOnLoad=true but X1 startup bias is neutral/unavailable. Waiting for fresh X1 crossover.");
        }
    }
    else
    {
        Print("ImmediateEntryOnLoad=false: waiting for fresh X1 crossover before enabling entries.");
    }
    ...
    if(g_AeronPublishModeEffective != AERON_PUBLISH_NONE)
    {
        ...
        if(g_AeronStreamIdCount <= 0)
        {
            Print("⚠️  No valid Aeron stream IDs configured. Disabling Aeron publishing for this run.");
            g_AeronPublishModeEffective = AERON_PUBLISH_NONE;
        }
        else
        {
            PrintFormat("Configured Aeron stream IDs: %s", StreamIdListToString(g_AeronStreamIds, g_AeronStreamIdCount));
        }
        ...
        if(g_AeronPublishModeEffective == AERON_PUBLISH_IPC_ONLY || g_AeronPublishModeEffective == AERON_PUBLISH_IPC_AND_UDP)
        {
            ...
            PrintFormat("  Stream IDs: %s", StreamIdListToString(g_AeronStreamIds, g_AeronStreamIdCount));
            ...
            if(!g_AeronIpcStarted)  // V20.8.3 - Check state before starting
            {
                ...
                for(int i = 0; i < g_AeronStreamIdCount; i++)
                {
                    ...
                    if(AeronBridge_StartPublisherIpcW(AeronPublishDir, AeronPublishChannelIpc, sid, 5000) != 0)
                    {
                        startedCount++;
                        PrintFormat("✅ IPC stream started: %d", sid);
                    }
                    else
                    {
                        PrintFormat("❌ Failed to start IPC stream: %d", sid);
                    }
                }
                ...
                if(startedCount > 0)
                {
                    ...
                    PrintFormat("✅ IPC Aeron publisher started for %d/%d streams", startedCount, g_AeronStreamIdCount);
                }
                else
                {
        ...
        if(g_AeronPublishModeEffective == AERON_PUBLISH_UDP_ONLY || g_AeronPublishModeEffective == AERON_PUBLISH_IPC_AND_UDP)
        {
            ...
            if(!g_AeronUdpStarted)  // V20.8.3 - Check state before starting
            {
                ...
                {
                    int sid = g_AeronStreamIds[i];
                    if(AeronBridge_StartPublisherUdpW(AeronPublishDir, AeronPublishChannelUdp, sid, 5000) != 0)
                    {
                        startedCount++;
                        PrintFormat("✅ UDP stream started: %d", sid);
                    }
                    else
                    {
                        PrintFormat("❌ Failed to start UDP stream: %d", sid);
                    }
                }
                ...
                if(startedCount > 0)
                {
                    ...
                    PrintFormat("✅ UDP Aeron publisher started for %d/%d streams", startedCount, g_AeronStreamIdCount);
                }
                else
                {
        ...
        if(g_AeronPublishModeEffective == AERON_PUBLISH_NONE)
        {
            Print("Mode: DISABLED_AT_RUNTIME | Status: ⚠️  DISABLED (invalid stream configuration)");
        }
        else if(g_AeronPublishModeEffective == AERON_PUBLISH_IPC_ONLY)
        {
            PrintFormat("Mode: IPC_ONLY | Status: %s", ipcSuccess ? "✅ OPERATIONAL" : "❌ DEGRADED");
        }
        else if(g_AeronPublishModeEffective == AERON_PUBLISH_UDP_ONLY)
        {
            PrintFormat("Mode: UDP_ONLY | Status: %s", udpSuccess ? "✅ OPERATIONAL" : "❌ DEGRADED");
        }
        else if(g_AeronPublishModeEffective == AERON_PUBLISH_IPC_AND_UDP)
        {
            string status = "";
...
void OnDeinit(const int reason)
{
    ...
    if(stochHandleX1 != INVALID_HANDLE)
    {
        IndicatorRelease(stochHandleX1);
        stochHandleX1 = INVALID_HANDLE;
    }

    if(stochHandleX2 != INVALID_HANDLE)
    {
        IndicatorRelease(stochHandleX2);
        stochHandleX2 = INVALID_HANDLE;
    }
...
void OnTick()
{
    ...
    if(!IsTradingAllowed())
    {
        ...
        if(EnableKillSwitch)
        {
            ...
            double mainLine[3], signalLine[3];
                if(SafeCopyBuffer(stochHandleX1, 0, 1, 3, mainLine, OP_INDICATOR) &&
                    SafeCopyBuffer(stochHandleX1, 1, 1, 3, signalLine, OP_INDICATOR))
            {
                double k0 = mainLine[0], d0 = signalLine[0];
                double k1 = mainLine[1], d1 = signalLine[1];
                ...
                if(reverseDetected && g_AeronPublishModeEffective != AERON_PUBLISH_NONE)
                {
    ...
    datetime currentBarTime = iTime(_Symbol, TimeframeX2, 0);
    if(currentBarTime == lastBarTime) return;
    lastBarTime = currentBarTime;

    UpdateAllPositionStatus();

    // X1 bias update based on fresh X1 crossover events
    bool x1BullishCross = false, x1BearishCross = false;
    if(!GetStochasticCrossSignals(stochHandleX1, x1BullishCross, x1BearishCross))
    {
        HandleError(OP_INDICATOR, GetLastError(), "Failed to read X1 stochastic buffers", false);
        return;
    }

    datetime x1SignalBarTime = iTime(_Symbol, TimeframeX1, 1);
    bool isNewX1SignalBar = (x1SignalBarTime != g_lastX1SignalBarTime);
    if(isNewX1SignalBar)
    {
        if(x1BullishCross)
        {
            ApplyX1Bias(X1_BIAS_LONG, false);
            g_lastX1SignalBarTime = x1SignalBarTime;
        }
        else if(x1BearishCross)
        {
            ApplyX1Bias(X1_BIAS_SHORT, false);
            g_lastX1SignalBarTime = x1SignalBarTime;
        }
    }

    if(!g_x1BiasReady || g_x1Bias == X1_BIAS_NONE)
    {
        static datetime lastNoBiasLog = 0;
        if(TimeCurrent() - lastNoBiasLog > 300)
        {
            Print("X1 bias not ready. Waiting for valid higher-timeframe direction.");
            lastNoBiasLog = TimeCurrent();
        }
        return;
    }

    bool x2BullishCross = false, x2BearishCross = false;
    if(!GetStochasticCrossSignals(stochHandleX2, x2BullishCross, x2BearishCross))
    {
        HandleError(OP_INDICATOR, GetLastError(), "Failed to read X2 stochastic buffers", false);
        return;
    }

    bool hasLongPositions = (scalpBuyOpened || trendBuyOpened);
    bool hasShortPositions = (scalpSellOpened || trendSellOpened);
    bool x2LongEntry = (g_x1Bias == X1_BIAS_LONG) && x2BullishCross;
    bool x2ShortEntry = (g_x1Bias == X1_BIAS_SHORT) && x2BearishCross;

    if(g_x1Bias == X1_BIAS_LONG && hasShortPositions)
    {
        Print("X1 LONG bias active while SHORT positions exist. Closing SHORT positions and waiting for X2 LONG trigger.");
        CloseAllSellPositions();
        UpdateAllPositionStatus();
        g_waitingForX2Trigger = true;
        return;
    ...
    if(g_x1Bias == X1_BIAS_SHORT && hasLongPositions)
    {
        Print("X1 SHORT bias active while LONG positions exist. Closing LONG positions and waiting for X2 SHORT trigger.");
        CloseAllBuyPositions();
        UpdateAllPositionStatus();
        g_waitingForX2Trigger = true;
        return;
    ...
    if(g_x1Bias == X1_BIAS_LONG && x2LongEntry && !hasLongPositions)
    {
        ...
        {
            Print("X2 LONG trigger detected but waiting for SELL positions to close.");
        }
    }

    if(g_x1Bias == X1_BIAS_SHORT && x2ShortEntry && !hasShortPositions)
    {
        ...
        {
            Print("X2 SHORT trigger detected but waiting for BUY positions to close.");
        }
    }
}
...
void OpenBuyPositions()
{
    ...
    // ===============================================
    if(g_AeronPublishModeEffective != AERON_PUBLISH_NONE && (scalpBuyOpened || trendBuyOpened))
    {
        double mainBuffer[3], signalBuffer[3];

        float confidence = 80.0;
          if(SafeCopyBuffer(stochHandleX2, 0, 0, 3, mainBuffer, OP_INDICATOR) &&
              SafeCopyBuffer(stochHandleX2, 1, 0, 3, signalBuffer, OP_INDICATOR))
        {
...
void OpenSellPositions()
{
    ...
    // V20.9 - Aeron Binary Signal Publishing with SIGNAL REVERSAL
    // ===============================================
    if(g_AeronPublishModeEffective != AERON_PUBLISH_NONE && (scalpSellOpened || trendSellOpened))
    {
        double mainBuffer[3], signalBuffer[3];
...
bool ExecuteImmediateTrade()
{
    ...
    if(!g_x1BiasReady || g_x1Bias == X1_BIAS_NONE)
    {
        Print("Immediate entry skipped: X1 bias is not ready.");
        return false;
    }

    double immediateMainLine[3], immediateSignalLine[3];
    if(!SafeCopyBuffer(stochHandleX2, 0, 1, 3, immediateMainLine, OP_INDICATOR) ||
       !SafeCopyBuffer(stochHandleX2, 1, 1, 3, immediateSignalLine, OP_INDICATOR))
    {
        Print("Immediate entry failed: Could not get indicator data safely.");
        return false;
    }

    double k0 = immediateMainLine[0], d0 = immediateSignalLine[0];
    double k1 = immediateMainLine[1], d1 = immediateSignalLine[1];
    bool x2BullishCross = (k0 < d0) && (k1 > d1);
    bool x2BearishCross = (k0 > d0) && (k1 < d1);

    if(g_x1Bias == X1_BIAS_LONG && x2BullishCross)
    {
        Print("Immediate entry condition: X1 LONG bias + X2 bullish crossover.");
        OpenBuyPositions();
        UpdateAllPositionStatus();
        if(scalpBuyOpened || trendBuyOpened)
        {
    ...
    else if(g_x1Bias == X1_BIAS_SHORT && x2BearishCross)
    {
        Print("Immediate entry condition: X1 SHORT bias + X2 bearish crossover.");
        OpenSellPositions();
        UpdateAllPositionStatus();
        if(scalpSellOpened || trendSellOpened)
        {
    ...
    {
        Print("Immediate entry skipped: X2 did not trigger in current X1 bias direction.");
        return false;
    }
}
...
bool CheckDailyProfitProtection()
{
    ...
    if(profitProtectionActive)
    {
        ...
        if(TimeCurrent() - lastProtectionLog >= 300)  // Print every 5 minutes
        {
