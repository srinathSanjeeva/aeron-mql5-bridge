//+------------------------------------------------------------------+
//|           Stochastic Straight Algo Strategy V20.10.mq5          |
...
#property version   "20.10"
#property description "V20.10 - X1/X2 Dual Timeframe Bias Trigger + Dual Session Manual Trading"
...
int OnInit()
{
    ...
    // Safe dual-indicator initialization with parameter validation
    if(K_Period_X1 <= 0 || K_Period_X1 > 1000 || D_Period_X1 <= 0 || D_Period_X1 > 1000 ||
       K_Period_X2 <= 0 || K_Period_X2 > 1000 || D_Period_X2 <= 0 || D_Period_X2 > 1000)
    {
        HandleError(OP_INIT, 0, 
                   StringFormat("Invalid indicator parameters: X1(K=%d,D=%d) X2(K=%d,D=%d)", K_Period_X1, D_Period_X1, K_Period_X2, D_Period_X2), true);
        return INIT_FAILED;
    }
    ...
    if(stochHandleX1 == INVALID_HANDLE || error != 0)
    {
        HandleError(OP_INIT, error, 
                   StringFormat("Failed to create X1 Stochastic indicator: K=%d, D=%d, TF=%d", 
                               K_Period_X1, D_Period_X1, TimeframeX1), true);
        return INIT_FAILED;
    }
    ...
    if(stochHandleX2 == INVALID_HANDLE || error != 0)
    {
        HandleError(OP_INIT, error,
                   StringFormat("Failed to create X2 Stochastic indicator: K=%d, D=%d, TF=%d",
                               K_Period_X2, D_Period_X2, TimeframeX2), true);
        return INIT_FAILED;
    }
    ...
    int wait_count = 0;
    while((!SafeCopyBuffer(stochHandleX1, 0, 0, 1, test_buffer, OP_INIT) ||
           !SafeCopyBuffer(stochHandleX2, 0, 0, 1, test_buffer, OP_INIT)) && wait_count < 5)
    {
        Sleep(100);
        wait_count++;
    }
    ...
    Print("✅ X1 and X2 stochastic indicators initialized successfully");
    ...
    if(ImmediateEntryOnLoad)
    {
        ...
        Print("Note: Trade will be placed only when X1 bias is valid and X2 confirms direction");
...
void OnDeinit(const int reason)
{
    ...
    {
        ResetLastError();
        if(!IndicatorRelease(stochHandleX1))
        {
            int error = GetLastError();
            Print("Warning: Failed to release X1 indicator handle, error: ", error);
        }
        else
        {
            Print("✅ X1 stochastic indicator released");
        }
    ...
    {
        ResetLastError();
        if(!IndicatorRelease(stochHandleX2))
        {
            int error = GetLastError();
            Print("Warning: Failed to release X2 indicator handle, error: ", error);
        }
        else
        {
            Print("✅ X2 stochastic indicator released");
        }
...
void OnTick()
{
    ...
    datetime currentBarTime = (datetime)SeriesInfoInteger(_Symbol, TimeframeX2, SERIES_LASTBAR_DATE);
    ...
    // X1 bias update from fresh X1 crossover events
    ...
    if(!g_x1BiasReady || g_x1Bias == X1_BIAS_NONE)
    {
        ...
        {
            Print("X1 bias not ready. Waiting for fresh higher-timeframe crossover.");
    ...
    if(g_x1Bias == X1_BIAS_LONG && hasShortPositions)
    {
        ...
        {
            Print("⚠️ HEDGING MODE WARNING: Failed to close all SELL positions after X1 flip.");
            return;
        }
        UpdateAllPositionStatus();
        g_waitingForX2Trigger = true;
    ...
    if(g_x1Bias == X1_BIAS_SHORT && hasLongPositions)
    {
        ...
        if(!closureSuccess && accountMode == MODE_HEDGING)
        {
            Print("⚠️ HEDGING MODE WARNING: Failed to close all BUY positions after X1 flip.");
            return;
        }
        UpdateAllPositionStatus();
        g_waitingForX2Trigger = true;
    ...
    if(g_x1Bias == X1_BIAS_LONG && x2BullishCross && !hasLongPositions)
    {
        if(WaitForClosureConfirmation && !AreAllReversePositionsClosed(true))
        {
            Print("X2 LONG trigger detected but waiting for SELL positions to fully close.");
            return;
        }
    ...
    if(g_x1Bias == X1_BIAS_SHORT && x2BearishCross && !hasShortPositions)
    {
        if(WaitForClosureConfirmation && !AreAllReversePositionsClosed(false))
        {
            Print("X2 SHORT trigger detected but waiting for BUY positions to fully close.");
            return;
        }
